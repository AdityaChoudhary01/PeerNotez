...
const markdownComponents = {
  ...
  em: ({ node, children, ...props }) => (
    <em style={{ color: 'rgba(255, 255, 255, 0.9)', fontStyle: 'italic' }} {...props}>
      {children}
    </em>
  ),
  ...
  li: ({ node, children, ...props }) => (
    <li style={{ marginBottom: '0.4rem', paddingLeft: '0.3rem' }} {...props}>
      {children}
    </li>
  ),
...
const FullBlogContent = ({ blog, onRefetch }) => {
  ...
  return (
    <main style={styles.articleContainer} className="blog-article-wrapper">
      ...
      <Link to="/blogs" style={styles.backLink} className="pn-back-link">
      ...
      <article style={styles.articleCard} className="blog-article-card">
        {optimizedBanner && (
          <img
            src={optimizedBanner}
            alt={`Banner for ${blog?.title || 'Blog post'}`}
        ...
        <div style={styles.articleMeta} className="blog-article-meta">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <StarRating rating={blog?.rating || 0} readOnly={true} size={14} />
            <span>({blog?.numReviews || 0} reviews)</span>
          </div>

          <span>{formattedDate ? `Published: ${formattedDate}` : ''}</span>
        </div>
      ...
      <section style={styles.secondarySection} aria-labelledby="learning-journey-title">
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', justifyContent: 'center' }}>
          <Link to="/search" style={styles.ctaBtn} className="pn-cta-btn pn-cta-primary">
          ...
          <Link
            ...
            className="pn-cta-btn pn-cta-secondary"
          >
      ...
      {blog?._id && <RelatedBlogs currentBlogId={blog._id} />}
...
const BlogPage = () => {
  ...
  const fetchBlogs = useCallback(
    async (signal) => {
      ...
      try {
        ...
        const { data } = await axios.get('/blogs', { params, signal });
        setBlogs(data?.blogs || []);
  ...
  const fetchSingleBlog = useCallback(
    async (signal) => {
      ...
      try {
        const { data } = await axios.get(`/blogs/${slug}`, { signal });
  ...
  useEffect(() => {
    ...
    if (slug) {
      fetchSingleBlog(controller.signal);
    } else {
      setSingleBlog(null);
    ...
    if (loadingSingle || !singleBlog) {
      return (
        <div
          ...
          aria-live="polite"
        >
          <FaSpinner className="fa-spin" style={{ marginRight: '10px' }} aria-hidden="true" /> Loading
          article...
        </div>
      );
    }
    ...
    <div style={styles.wrapper}>
      ...
      <header style={styles.header} className="blog-list-header">
        <h1 style={styles.title}>
          <FaFeatherAlt aria-hidden="true" /> The PeerNotez Blog
        </h1>
        <p style={styles.subtitle}>Deep dive into study hacks, developer insights, and community stories.</p>
        <Link to="/blogs/post" style={styles.ctaBtn} className="pn-cta-btn pn-cta-primary">
      ...
      <section style={styles.controls} className="blog-list-controls" aria-label="Search and filter blogs">
        <form onSubmit={handleSearchSubmit} style={styles.searchForm} className="blog-search-form">
          ...
          />
          <button type="submit" style={styles.searchBtn} aria-label="Perform search">
            <FaSearch />
          </button>
        </form>

        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <label htmlFor="blog-sort-select" style={{ color: 'rgba(255,255,255,0.7)' }}>
            Sort by:
          </label>
          <select
            id="blog-sort-select"
      ...
      <main>
        {loading ? (
          <div style={{ textAlign: 'center', padding: '2rem', color: 'rgba(255,255,255,0.7)' }} aria-live="polite">
            Loading blog posts...
          </div>
        ) : blogs.length > 0 ? (
          <>
            <div style={styles.grid}>{blogs.map((blog) => <BlogCard key={blog._id} blog={blog} />)}</div>
            <Pagination page={page} totalPages={totalPages} onPageChange={setPage} />
          </>
        ) : (
          <p style={{ textAlign: 'center', marginTop: '2rem', color: 'rgba(255,255,255,0.5)' }}>
